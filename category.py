...
class CategoryRandomizer:
    ...
    def display_randomizing_ui(self):
        ...
        self.category_box = tk.Label(
            self.window, text="...", font=("Comic Sans MS", 50, "bold"), 
            fg="#F88379", bg="#FCE0D6", width=20, height=2, relief="solid"
        )
    ...
    def update_category(self):
        if not self.randomizing_active:
            return  
        if self.category_box.winfo_exists():
            ...
            self.after_id = self.window.after(100, self.update_category)
        else:
    ...
    def finish_randomizing(self):
        ...
        category_label = tk.Label(
            self.window, text=f"Final Category: {self.selected_category}", 
            font=("Comic Sans MS", 40, "bold"), fg="#FF6347", bg="#FCE0D6"
        )
